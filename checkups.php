...
<?php
    ...
    $datei = fopen("files/psites.txt","r");                                                                                              
    $freelist = fread($datei,99999);
    fclose($datei);
    include_once("files/crypto.php");
    ...
    foreach ($nms as $nombre => $item){
     ...
     /*   if (strpos($title, 'Asianomics') !== false) {                                                                                        //Had to write a special function for Asianomics to work
            ...
            $cachinfo=true;
            if (file_exists("files/".$ydate."checkfileb".$code.".txt")) {
                $datei = fopen("files/".$ydate."checkfileb".$code.".txt","r");
            ...
            if (file_exists("files/".$ydate."checkfileb".$code.".txt")) {
                ...
                if((strcmp($checkText, $check) != 0)and(strcmp(trim($checkText), "") != 0)){   //*/             //MOVING UP
                    ...
                    echo '<script type="text/javascript"> window.dataArr'.$num.' = '.json_encode($dArr).'; </script>';
                      ...
                      //*/
            echo "</div>";
            if (file_exists("files/".$pdate."checkfileb".$code.".txt")) {
                unlink("files/".$pdate."checkfileb".$code.".txt");        
            }
            echo "<script type='text/javascript'>loaded(".$num.");</script>";
            $success = false;

            $checkfileb = fopen("files/".$tdate."checkfileb".$code.".txt","w");
